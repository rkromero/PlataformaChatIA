    ...
    <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {channels.map((ch) => {
              ...
              return (
                <div
                  key={ch.id}
                  className="group rounded-xl border border-white/[0.06] bg-surface-2 p-4 transition-shadow duration-150 hover:shadow-md"
                >
      ...
      {hasOfficialApi && (
        <section>
          ...
          {templates.length === 0 ? (
            <div className="rounded-xl border-2 border-dashed border-white/[0.08] p-8 text-center">
            ...
            <div className="divide-y divide-white/[0.06] rounded-xl border border-white/[0.06] bg-surface-2">
              {templates.map((tpl) => {
                ...
                }[tpl.status] ?? 'bg-white/5 text-gray-500';
                const statusLabel = {
                  APPROVED: 'Aprobada',
                  PENDING: 'Pendiente',
                  REJECTED: 'Rechazada',
                ...
                return (
                  <div key={tpl.id} className="flex items-start gap-4 px-4 py-3.5">
                    <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg bg-indigo-500/10">
                      <svg className="h-4.5 w-4.5 text-indigo-400" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                      </svg>
                    </div>
                    <div className="min-w-0 flex-1">
                      ...
                      {body && (
                        <p className="mt-1 line-clamp-2 text-xs leading-relaxed text-gray-400">{body}</p>
                      )}
                    </div>
          ...
          <div className="mt-4 rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
            <div className="flex gap-3">
              ...
              <div>
                <p className="text-xs font-semibold text-gray-300">¿Cómo funcionan las plantillas?</p>
                <ul className="mt-1.5 space-y-1 text-xs leading-relaxed text-gray-400">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Link
            ...
            className="group cursor-pointer rounded-xl border-2 border-white/[0.06] p-5 transition-all duration-150 hover:border-emerald-500"
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-gray-100">WhatsApp API Oficial</p>
                <p className="text-xs text-gray-400">Meta Business Platform</p>
              </div>
            </div>
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              <span className="rounded-full bg-emerald-500/10 px-2 py-0.5 text-[10px] font-medium text-emerald-400">Oficial</span>
              <span className="rounded-full bg-emerald-500/10 px-2 py-0.5 text-[10px] font-medium text-emerald-400">Templates HSM</span>
              <span className="rounded-full bg-emerald-500/10 px-2 py-0.5 text-[10px] font-medium text-emerald-400">Sin riesgo</span>
            </div>
          </Link>

          <Link
            href="/dashboard/channels/connect-qr"
            className="group cursor-pointer rounded-xl border-2 border-white/[0.06] p-5 transition-all duration-150 hover:border-purple-500"
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-gray-100">WhatsApp QR</p>
                <p className="text-xs text-gray-400">Escanear código QR</p>
              </div>
            </div>
            <p className="mt-3 text-sm leading-relaxed text-gray-400">
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              <span className="rounded-full bg-purple-500/10 px-2 py-0.5 text-[10px] font-medium text-purple-400">Rápido</span>
              <span className="rounded-full bg-purple-500/10 px-2 py-0.5 text-[10px] font-medium text-purple-400">Sin verificación</span>
              <span className="rounded-full bg-amber-500/10 px-2 py-0.5 text-[10px] font-medium text-amber-400">No oficial</span>
            </div>
          </Link>
        </div>
      </section>
...
function TemplateDeleteButton({ templateId }: { templateId: string }) {
  return (
    <form action={async () => {
      ...
      <button
        type="submit"
        className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg text-gray-600 transition-colors duration-150 hover:bg-red-500/10 hover:text-red-400"
